...
- [ ] E3-B.2 按[普通任务反馈进化合同](docs/design/ordinary-feedback-evolution-v1.md)分片实现：a 持久终态触发与重启扫描、b 公开反馈→模型 R0 规划→候选、c 候选→独立配对→拒绝／晋升→guard／回滚→后续真实加载收据，机制已接到普通 Main／CLI。首次[真实普通入口](experiments/ordinary_feedback_live/RESULTS.md)与[同任务族探针](experiments/ordinary_feedback_live/WORKBENCH_ALIGNED_RESULTS.md)的父、候选均失败而拒绝；失败整图现不能原样采用。[图绑定修复后探针](experiments/ordinary_feedback_live/WORKBENCH_BINDING_REPAIR_RESULTS.md)来源交付及 selection 两臂仍失败。普通流程现可选择持久化的有界 O 搜索并保留直接 O/S 补丁；[首次真实搜索探针](experiments/ordinary_feedback_live/WORKBENCH_SEARCH_FLOW_RESULTS.md)由 R0 选中搜索且执行两次 MiMo 修复，但两次均因补丁合同无效而未产生候选，开发资格评估、selection、guard、复用未达。下一步把有界公开验证错误接回修复器，打通可执行候选及后续独立评价；任务族路由和多任务净效果对照也未完成。E 阶段完整出口仍未过。
