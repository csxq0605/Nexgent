...
独立 `FourArmStudyService` 已实现 F/S/M/E 同一 final-holdout 统计单位的原子预留、冻结 package/M release 与环境身份、四臂顺序、TaskService 执行和崩溃后保守恢复。它不改写两臂研究合同；真实 final-holdout 尚未消耗。四臂统计评估、Holm 校正、外层搜索成本、记忆实际检索激活、公开投影与 CLI/UI 仍待实现；如声称 O/S 与 M 交互效应，还需预登记第五臂。详见[四臂确认协议](docs/research/four-arm-confirmatory-protocol-v1.md)。
...
| 冻结正式研究 P5 | 两臂 final-holdout 执行器与独立四臂 quartet 原子预留/执行/保守恢复底座已有定向测试；四臂统计推断、outer cost、memory 检索激活、CLI/信息窗接入与外部多任务族真实研究待完成 |
