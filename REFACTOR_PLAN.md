...
- [ ] E3-B.2 按[普通任务反馈进化合同](docs/design/ordinary-feedback-evolution-v1.md)分片实现：a 持久终态触发与重启扫描、b 公开反馈→模型 R0 规划→候选、c 候选→独立配对→拒绝／晋升→guard／回滚→后续真实加载收据，机制已接到普通 Main／CLI。首次[真实普通入口](experiments/ordinary_feedback_live/RESULTS.md)与[同任务族探针](experiments/ordinary_feedback_live/WORKBENCH_ALIGNED_RESULTS.md)的父、候选均失败而拒绝；失败整图现不能原样采用。[图绑定修复后探针](experiments/ordinary_feedback_live/WORKBENCH_BINDING_REPAIR_RESULTS.md)来源交付及 selection 两臂仍失败。普通流程现可选择持久化的有界 O 搜索并保留直接 O/S 补丁；[首次真实搜索](experiments/ordinary_feedback_live/WORKBENCH_SEARCH_FLOW_RESULTS.md)两次补丁无效、未进资格评估；[固定错误码后同题复测](experiments/ordinary_feedback_live/WORKBENCH_SEARCH_REPAIR_RESULTS.md)首次生成有效 O 候选并在新 development seed 14 跑配对资格评估，但候选静态图错误、父图交付错误、第二次修复图 JSON 无效，未获 selection 资格。候选 `ask` 参数已能在配对前静态拒绝；[其后真实探针](experiments/ordinary_feedback_live/WORKBENCH_PREFLIGHT_RESULTS.md)因来源 DNS 故障、候选生成失败未检验该预检，还发现基础设施失败被错误送进 RSI。入口现拒绝基础设施或计量不完整来源。下一步对完整来源检验候选预检和资格修复；任务族路由、guard／复用真实正例和多任务净效果对照仍缺。E 阶段完整出口未过。
- [x] E3-B.2 候选开发错误投影：宿主把三类已知执行失败压成固定修复码，拒绝基础设施失败及不完整计量的配对证据；编排 41 项、生成修复 17 项定向测试通过。它只推进负反馈修复，不算 E3-B.2 或 E 阶段出口。
