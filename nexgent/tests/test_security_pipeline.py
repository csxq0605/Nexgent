...
from nexgent.security_pipeline import (
    sanitize_output,
...
class TestClassifyActionModel:
    ...
    def setup_method(self):
        from nexgent import security_pipeline
    ...
    def test_cache_hit(self):
        ...
        # Verify cache was populated (cache key format: tool:working_dir:permission_mode:args_hash)
        args_hash = hashlib.md5(json.dumps({"command": "ls"}, sort_keys=True).encode()).hexdigest()[:12]
        cache_key = f"run_command::default:{args_hash}"  # working_dir="" and permission_mode="default"
...
class TestCacheEviction:
    ...
    def test_classifier_cache_eviction(self):
        ...
        try:
            ...
            # Trigger eviction by calling classify_action
    ...
    def test_review_cache_eviction(self):
        ...
        try:
            ...
            # Verify cache has 3 entries (exceeds max size of 2)
            assert len(security_pipeline._review_cache) == 3
            # Manually trigger eviction (same logic as in review_action)
            with security_pipeline._review_cache_lock:
                if len(security_pipeline._review_cache) >= security_pipeline._REVIEW_CACHE_MAX_SIZE:
                    expired = [k for k, (t, _) in security_pipeline._review_cache.items() if now - t >= security_pipeline._REVIEW_CACHE_TTL]
                    for k in expired:
                        del security_pipeline._review_cache[k]
            # Cache should have been evicted - expired entries removed
            assert len(security_pipeline._review_cache) == 0  # all were expired
        finally:
