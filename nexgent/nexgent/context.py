...
_logger = logging.getLogger("nexgent.context")
...
def load_memory(project_dir: str) -> str:
    ...
    - MEMORY.md index is ALWAYS loaded (first 500 lines / 50KB)
    ...
    if os.path.exists(memory_index_path):
        try:
            ...
            if len(lines) > MEMORY_INDEX_MAX_LINES:
                lines = lines[:MEMORY_INDEX_MAX_LINES] + [f"... [truncated to {MEMORY_INDEX_MAX_LINES} lines]\n"]
            content = "".join(lines).strip()
            if content:
