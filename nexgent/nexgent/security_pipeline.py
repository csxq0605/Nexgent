...
def classify_action_model(
    ...
    # Include working_dir and permission_mode to avoid cross-context cache hits
    args_hash = hashlib.md5(json.dumps(tool_args, sort_keys=True, ensure_ascii=False).encode()).hexdigest()[:12]
    cache_key = f"{tool_name}:{working_dir}:{permission_mode}:{args_hash}"
    now = time.time()
...
Is this decision appropriate? Are there any overlooked risks?"""
    ...
    try:
        ...
        try:
            ...
            if salvaged is None:
                logging.getLogger(__name__).warning(
                    "Review action: failed to parse JSON, skipping review"
                )
                return None
            parsed = salvaged
        result = ReviewResult(
