...
| Four-arm benchmark | A noncausal F/S/M/E qualification harness shares task inputs and hard caps. A separate formal four-arm service now atomically reserves final-holdout quartets and freezes package, evaluator and accepted M releases, with fail-closed recovery in deterministic tests. No real final quartet has run. | Formal execution substrate, not a four-arm causal estimate or RSI benefit. |
...
| G4 Four-arm protocol | One preregistered F/S/M/E schedule binds the same source clusters, model, hard budget, evaluator, memory namespaces, outer evolution cost, missingness rule, and multiple-comparison policy. | Quartet reservation/execution/recovery core passes deterministic tests. Statistics, outer cost ledger, memory-use activation, public projection and real holdout run remain open. |
