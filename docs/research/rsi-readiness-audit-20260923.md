...
| O/S generation | MiMo v2.6-flash produced two valid S candidates. The first changed a publishing skill, but its changed branch did not execute; it failed selection. The second changed an adjudicator prompt and loaded on development and selection tasks. Its development 2/2 vs parent 0/2 contrast did not replicate on fresh selection: both scored 1/2, and the candidate was rejected. | Real S generation and activation, not demonstrated improvement or O evolution. |
| Pure M | MemoryService can admit, select, promote, snapshot, and consume a separate M release in deterministic tests. | Engineering contract only until default R0 produces a real M candidate and later tasks consume it under independent evaluation. |
...
| G2 O/S activation | A real generated O change, with role/workflow edges actually exercised, passes development checks and a new paired selection without protocol regression. | Not met. A second real S candidate loaded but yielded no selection improvement; no real O candidate yet. |
| G3 M activation | Default R0 generates an M proposal; frozen M release is consumed on later independent tasks against the same package with empty memory. | Deterministic route exists; real-model effect not measured. |
