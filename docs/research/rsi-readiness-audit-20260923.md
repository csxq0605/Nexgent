...
| Four-arm benchmark | A noncausal F/S/M/E qualification harness shares task inputs and hard caps. The separate formal service atomically reserves final-holdout quartets and freezes package, evaluator and accepted M releases, with fail-closed recovery. A sealed-record assessor now implements cluster tests and Holm correction in synthetic tests. No real final quartet has run. | Formal execution and analysis substrate; no observed four-arm effect or RSI benefit. |
...
| G4 Four-arm protocol | One preregistered F/S/M/E schedule binds the same source clusters, model, hard budget, evaluator, memory namespaces, outer evolution cost, missingness rule, and multiple-comparison policy. | Quartet execution and sealed-record statistics pass deterministic tests. Outer cost ledger, memory-use activation, lineage/actual component activation, public projection and real holdout run remain open. |
