...
后续实现将完整对象加尾随文本分类为唯一可恢复的格式错误；原响应仍记 invalid，宿主在计量完整时用独立持久 RPC／模型收据要求第二次模型返回完全相同的对象。两次调用均计入原预算，未知调用不重发；其它 JSON 错误、内容改变或计量不完整均拒绝。普通任务的[纵向集成测试](../../tests/test_ordinary_rsi_full_flow.py)在确定性模型边界下真实经过 Generation、development 配对、selection、guard、晋升及新 TaskService 复用，联合定向测试 98 项通过。它解决“服务是否实际接通”的工程问题；AutoSci／AFlow 式模型搜索收益、真实 MiMo 的稳定合格候选和多任务对照仍无证据。
...
[额度修正后的真实普通任务](../../experiments/ordinary_feedback_live/WORKBENCH_ARCHITECT_BUDGET_RESULTS.md)越过上一轮的 architect 截断，但来源任务仍因交付 schema 失败。R0 选中 O 搜索：首个补丁违反 `replace` 合同，第二次调用在 Provider 180 秒超时且计量不完整，搜索终止，未进入 development／selection。与 AutoSci SciEvolve、AFlow 的候选搜索目标对照，这一收据只表明前端资源瓶颈改变，**没有**有效候选的独立竞争或跨任务采用；下一项验证要以真实候选进入配对、独立选拔、guard 和复用为连续证据，而非增加局部机制计数。
