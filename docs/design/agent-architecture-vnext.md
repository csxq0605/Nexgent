...
目标分层为[无状态 Kernel、通用 Compiler/Runtime 与任务专一型自设计团队](stateless-kernel-and-self-designed-teams.md)。此处 AgentPackage/ExecutablePlan 是现行版本化执行载体，不代表固定三角色包、预写 workflow 或现有 TaskService 就已经实现了按任务自主组队和能力开发；任务与进化状态应持久化于通用 Store，而非由 Kernel 私有持有。
...
| --- | --- | --- |
| Task Runtime | 加载指定包，恢复 Episode，执行已准入的计划节点，管理工件和子任务 | 不承载任务策略；运行算法本身是宿主版本 |
...
OpenFOAM demo 的验收范围由 [独立领域设计](../demos/openfoam-cfd-design.md) 规定：二维顶盖驱动方腔、输入案例、允许操作、作业成功条件、数值有效性、比较指标与总计算限制。本机 WSL2 / Ubuntu 20.04 / Foundation 8 的只读确认见 [环境记录](../demos/openfoam-environment-20260916.md)。环境存在不等于 Nexgent 插件已实现或算例已通过；本轮没有运行仿真。任务是否完成、求解器是否正常退出以及物理结论是否成立必须分别表示，流体算法不进入本文的核心合同。
...
每阶段必须有从用户输入到成果的完整路径；不是先把所有抽象目录建完。阶段编号与 [重构计划](../../REFACTOR_PLAN.md) 一致，P1 细分两个可交付片段。本轮仅 P0 文档工作；其余均未执行。
