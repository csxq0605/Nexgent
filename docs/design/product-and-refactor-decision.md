...
日期：2026-09-16；界面补充决策：2026-09-22。状态：核心定位与 vNext 设计已固定，任务运行时已分阶段落地；主对话界面正在替换原任务控制台。0.8 的代码、数据与实验结论保持原样。

## 1. 不变的产品要求

> nexgent才是RSI的智能体框架，demo是来证明的！！！nexgent本质上应该是能跑benchmark的框架级智能体架构，你不能这样耦合；目的是这个框架，不是对demo特化

**Nexgent 是能够自主组织智能体完成任务、根据反馈更新自身工作方式，并通过 benchmark 检验这些更新的通用 RSI 智能体框架。**

这里的“通用”进一步限定为**无状态、任务类型无关的 Kernel 与通用 Compiler/Runtime**：任务状态、记忆和版本保存在外部 Store，由版本化输入交给运行器；Kernel 不内置领域阶段或固定角色。Nexgent 的差异性主体是多智能体依据任务自行设计、实现并改进任务专一团队，而不只是执行人写好的 workflow。完整边界见[无状态底座与自设计团队](stateless-kernel-and-self-designed-teams.md)。
...
AutoSci 提供任务生命周期、结构化记忆、技能和编排更新的组织参考；ADAS/AFlow 提供完整 agent/workflow 的行为搜索方法；DGM 提供改进后智能体继续执行与探索档案；Hyperagents/STOP 提供改进过程可更新及后代效用检验。
...
参考 AutoSci 的对话式技能入口和持久研究记忆，以及本机 Manyselves 的 “Users talk to Main only” 结构，Nexgent 默认启动 `MainWindow`。Main 对话与 TaskService 使用同一执行路径；OpenFOAM、scientific-discovery、BBH 和其他 benchmark 只通过插件提供任务与评价，不改变主界面或核心状态机。高级控制台仍然可打开，用于审计和研究，不再承担普通用户的第一步交互。

所有面向用户的品牌文本统一写作 `Nexgent`；`NEXGENT` 只作为环境变量、包兼容名和历史记录保留。

## 9. 本轮交付和文档优先级

本轮固定定位、架构、论文到方法的对应、迁移计划、OpenFOAM demo 设计和 Main 对话界面决策；新界面以小切片实现，不把尚未完成的真实模型研究写成已完成结论。

阅读优先级：用户不变要求 → 本决策中的产品边界 → [vNext 架构](agent-architecture-vnext.md)及领域独立合同 → [分阶段计划](../../REFACTOR_PLAN.md)。历史运行记录对历史事实仍然有效；旧源码文件限制和旧“整体完成”措辞不再定义 vNext 范围。
