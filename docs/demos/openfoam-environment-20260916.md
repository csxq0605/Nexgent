...
这份 **2026-09-16** 记录当时只证明安装位置、版本和教程存在；当次没有运行 `blockMesh`、`checkMesh`、`icoFoam`、`postProcess` 或任何 case，也没有安装、升级、修改 WSL 配置、启动 Docker 服务或调用模型。后续执行结果必须与这次只读核查按日期分开解释。
...
2026-09-20 的后续工作已经实现独立插件，并在同一 WSL2 `Ubuntu-20.04` / Foundation 8 环境真实运行 Re=10、20×20×1 smoke。`blockMesh`、`checkMesh`、`icoFoam` 均成功，最新 `U`/`p` 字段完成解析，隐藏评价器接受交付。后续证据见[真实 smoke 验证记录](openfoam-smoke-validation-20260920.md)和[机器可读收据摘要](openfoam-smoke-receipt-20260920.json)；它不改变本页对 9 月 16 日核查动作的历史陈述。
...
## 6. 2026-09-16 时仍未知的事项与后续状态

| 9 月 16 日尚未知 | 9 月 20 日状态或正式执行前条件 |
| --- | --- |
| 求解器实际启动和退出状态 | Re=10 smoke 已解决：三个固定命令返回 0，时间、输出字节数与完整输出摘要已保存；完整记录另页保留 |
...
| 完整 agent 软件与 OpenFOAM 能力适配 | 独立 smoke 插件、宿主管理工作区和隐藏评价闭环已实现；固定无模型 AgentPackage 已通过真实 `TaskService` 受控恢复 episode。模型驱动 episode、多角色 CFD 工作流和正式数值验证尚未完成 |
| 聚变相关环境/验证 | 用户说明没有聚变环境；本轮未进行全盘软件普查，不能由 cavity 推出聚变能力 |

上述是按时间更新的进入条件，不是用户待填写的确认清单。9 月 16 日没有发起仿真、模型运行或安装；9 月 20 日仅发起不调用模型的已冻结 smoke，未执行正式 Re=100、收敛或 RSI 实验。
