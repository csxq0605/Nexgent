...
P1 的确定性合同测试不调用真实模型供应商，也不构成真实任务成功、统计效果或 RSI 改进的证据。P1 真实模型验证会产生外部供应商载荷，目前等待用户明确授权；获得授权后须单独记录供应商、模型、预算、任务分布、实际收据和验收结果。P2 的真实 OpenFOAM 求解器 smoke 也不能代替该模型证据。
